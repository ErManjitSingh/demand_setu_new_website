...
export default function BookingCheckoutForm({ listing, searchParams }) {
  const checkIn = parseDateParam(searchParams.checkIn);
  const checkOut = parseDateParam(searchParams.checkOut);
  ...
  const nightly = Number(searchParams.price) || listing.price;
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(280px,360px)_minmax(0,1fr)] lg:items-start lg:gap-8">
      ...
      <aside className="space-y-4 lg:sticky lg:top-24">
        ...
        <div className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
          ...
          <ul className="mt-3 space-y-2 text-sm">
            ...
            {memberSignIn && (
              <PriceRow
                ...
                value={`−${formatPrice(Math.round(total * 0.1))}`}
                accent
              />
            )}
          </ul>
          ...
          <p className="mt-2 text-[11px] text-muted">Includes taxes and fees · INR</p>
        </div>
      </aside>
