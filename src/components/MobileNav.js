...
const tabs = [
  { href: "/", label: "Home", match: (p, q) => p === "/" },
  { href: "/listings", label: "Explore", match: (p, q) => p === "/listings" && !q.get("category") },
  { href: "/listings?category=hotel", label: "Hotels", match: (p, q) => q.get("category") === "hotel" },
  { href: "/listings?category=homestay", label: "Villas", match: (p, q) => q.get("category") === "homestay" },
];
...
export default function MobileNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 px-4 pb-[max(0.5rem,var(--safe-bottom))] md:hidden"
    >
      <ul className="mx-auto flex max-w-md items-stretch justify-around gap-1 rounded-2xl border border-white/60 bg-white/90 p-1.5 shadow-2xl shadow-stone-900/15 backdrop-blur-xl">
        {tabs.map((tab) => {
          ...
          return (
            <li key={tab.href} className="flex-1">
              <Link
                href={tab.href}
                className={`flex flex-col items-center gap-0.5 rounded-xl px-2 py-2 text-[10px] font-bold transition ${
                  isActive
